...
def create_mesh(
    ...
    quads: bool = True,
    semi_structured_quad: bool = True,
    second_order: bool = True,
):
    """Generate the FSI2 benchmark mesh (solid flag + ALE fluid domain) and
    write it to ``data/meshes/fsi2/mesh<suffix>.xdmf``.

    Parameters
    ----------
    fine:
        Use a finer base resolution.
    coarse:
        Deliberately coarser mesh, for fast smoke-testing only (not for
        demo/production use). Scales the resolution parameters up by
        ``coarse_factor`` and appends a "_coarse" suffix to the output
        filename, so it never overwrites the regular meshes.
    coarse_factor:
        Factor by which resolutions are scaled up when ``coarse`` is set.
    quads:
        Recombine the mesh into quadrilateral cells.
    semi_structured_quad:
        Use the quasi-structured quad algorithm instead of
        Frontal-Delaunay-for-quads + recombine. Only relevant when ``quads``
        is set.
    second_order:
